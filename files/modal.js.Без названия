var createFactory = function() {
    ...
    var modal = function(params, $existingModal) {
        ...
        var closeButtonHtml = '<div class="close-btn"><i class="fa fa-times"></div>';

		if ($existingModal) {
			var $modal = $existingModal;
			createdFromExisting = true;
		} else {
        var $modal = $("<div class='modal gc-modal' id='gc-modal-"+(initVal++)+"'><div class='modal-dialog'><div class='modal-content'><div class='modal-body'>" + [
                '<div class="gc-modal-content" id="gc-modal-content-'+(initVal++)+'">',
                loaderHtml,
                '</div>'
            ].join('') + "</div></div></div>"+closeButtonHtml+"</div>");
		}
